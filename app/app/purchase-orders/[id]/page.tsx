...
export default function PurchaseOrderViewPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const [q, prof] = await Promise.all([getPurchaseOrder(id), fetchProfile()]);
        if (cancelled) return;
        if (!q) {
  ...
  const { subtotal, taxTotal, discount, shipping, total } =
    computePurchaseOrderTotals(purchaseOrder);

  const from = purchaseOrder.from_snapshot as {
    type?: string;
    company_name?: string;
    full_name?: string;
    email?: string;
  ...
  const bill = purchaseOrder.bill_to_snapshot as {
    type?: string;
    company_name?: string;
    full_name?: string;
    email?: string;
            ...
            <div className="grid sm:grid-cols-2 gap-8">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="font-medium">{fmtDate(purchaseOrder.issue_date)}</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-muted-foreground">
                    Valid Until
                  </p>
                  <p className="font-medium">{fmtDate(purchaseOrder.valid_until)}</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-muted-foreground">
                    Status
