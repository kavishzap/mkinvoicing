...
import { Copy, Download, Edit, FileInput, Printer } from "lucide-react";
...
function buildFromLines(po: PurchaseOrderDetail, prof: Profile | null | undefined, branding: DocumentBranding) {
  const fromSnap = (po.from_snapshot ?? {}) as Record<string, unknown>;
...
export function PurchaseOrderViewActions({
  ...
  const handleDuplicate = () => {
    if (isRunning) return;
    router.push(
      `/app/purchase-orders/new?duplicate=${encodeURIComponent(purchaseOrderId)}`
    );
  };
  ...
  const renderPdf = async (mode: "download" | "print") => {
    ...
    try {
      const po = purchaseOrder ?? (await getPurchaseOrder(purchaseOrderId));
      ...
      const fromSnap = po.from_snapshot ?? {};
      const acctType = prof?.accountType ?? "individual";
      const fallbackName =
      ...
      doc.setFont("helvetica", "bold").setFontSize(22).text("PURCHASE ORDER", rightX, 30, {
        align: "right",
        baseline: "middle",
      });
      doc.setFont("helvetica", "normal").setFontSize(10);
      doc.text(String(po.number || ""), rightX, 48, { align: "right" });
      ...
      const fromLines = buildFromLines(po, prof, branding);
      ...
      const bill = po.bill_to_snapshot ?? {};
      const billName =
      ...
      const issue = new Date(po.issue_date).toLocaleDateString("en-GB");
      const valid = new Date(po.valid_until).toLocaleDateString("en-GB");
      const detailsLines = [
        `Issue Date: ${issue}`,
        `Valid Until: ${valid}`,
        `Status: ${po.status}`,
      ...
      const bodyRows: RowInput[] = [...(po.items ?? [])]
        .sort(
          (a, b) =>
        ...
        return [
          ...
          it.description ?? "",
          String(qty),
          money(unit, po.currency),
          `${it.tax_percent ?? 0}%`,
          money(lineTotal, po.currency),
        ];
      });
      ...
      doc.text(money(totals.subtotal, po.currency), valueX, ty, { align: "right" });
      ty += 16;
      doc.text("Tax", labelX, ty);
      doc.text(money(totals.taxTotal, po.currency), valueX, ty, { align: "right" });
      ty += 16;
      if (totals.discount > 0) {
        doc.text("Discount", labelX, ty);
        doc.text("-" + money(totals.discount, po.currency), valueX, ty, { align: "right" });
        ty += 16;
      }
      if (totals.shipping > 0) {
        doc.text("Shipping", labelX, ty);
        doc.text(money(totals.shipping, po.currency), valueX, ty, { align: "right" });
        ty += 16;
      }
      ...
      doc.text(money(totals.total, po.currency), valueX, ty, { align: "right" });

      let ny = ty + 36;
      const notes = po.notes ? String(po.notes) : "";
      const terms = po.terms ? String(po.terms) : "";
      ...
      const filename = `PurchaseOrder-${po.number || purchaseOrderId}.pdf`;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Button type="button" variant="default" className="gap-2" asChild>
        <Link
          href={`/app/purchase-invoices/new?convertFromPurchaseOrder=${encodeURIComponent(
            purchaseOrderId
          )}`}
        >
      ...
      <Button type="button" variant="outline" className="gap-2" asChild>
        <Link href={`/app/purchase-orders/${purchaseOrderId}/edit`}>
