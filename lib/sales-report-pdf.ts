...
export async function generateSalesReportPDF(
  ...
  autoTable(doc, {
    ...
    body: [
      ["Cash (collected)", formatCurrency(data.salesBreakdown.cash, data.currency)],
      ["Credit (collected)", formatCurrency(data.salesBreakdown.credit, data.currency)],
      ["Bank / Juice (collected)", formatCurrency(data.salesBreakdown.bankJuice, data.currency)],
      ["Due (outstanding)", formatCurrency(data.salesBreakdown.due, data.currency)],
      ["Total sales", formatCurrency(data.totalSalesGross, data.currency)],
    ],
    margin: { left: M, right: M },
    theme: "plain",
    headStyles: {
