...
export async function createCreditNote(
  ...
  const wantsPost = cn.status === "posted";
  const companyId = await requireActiveCompanyId();
  ...
  const { data, error } = await supabase.rpc("create_credit_note", {
    p_credit_note: {
      ...
      // Always insert as draft; post_credit_note requires draft status.
      status: "draft",
  ...
  if (wantsPost) {
    await postCreditNote(creditNoteId);
  }
...
export async function deleteCreditNote(id: string): Promise<void> {
  ...
  const { error } = await supabase.rpc("delete_credit_note", {
    p_credit_note_id: id,
  });

  if (error) {
    // Fallback when RPC not deployed yet (draft/cancelled only).
    if (
      error.message?.includes("delete_credit_note") ||
      error.code === "PGRST202"
    ) {
      if (wasPosted) {
        throw new Error(
          "Posted credit notes require sql/patch_credit_notes_delete_posted.sql in Supabase.",
        );
      }
      const { error: delErr } = await supabase
        .from("credit_notes")
