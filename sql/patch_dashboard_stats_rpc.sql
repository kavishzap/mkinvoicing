-- One round-trip for /app dashboard stats (replaces paging through all invoices/expenses/etc.).
...
COMMENT ON FUNCTION public.get_dashboard_stats(uuid, int) IS
  'Dashboard aggregates for /app; matches app filters (invoices: company_id = p or null).';
